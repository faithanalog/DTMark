  ...
  RootPanel(this.game) {
    ...
    if (!BaseGame.touchSupport) {
      canvas.onMouseDown.listen((evt) {
        ...
        _mouseY = y;
        if (active) {
          _mouseDown(new MouseDownEvent(this, x, y, evt.button));
        }
      });
      canvas.onMouseUp.listen((evt) {
        ...
        _mouseY = y;
        if (active) {
          _mouseUp(new MouseUpEvent(this, x, y, 0));
        }
      });
      canvas.onMouseMove.listen((evt) {
        ...
        _mouseY = y;
        if (active) {
          _mouseMove(new MouseMoveEvent(this, x, y));
        }
      });
    } else {
      canvas.onTouchStart.listen((evt) {
        ...
        _mouseY = y;
        if (active) {
          _mouseDown(new MouseDownEvent(this, x, y, 0));
        }
      });
    ...
    canvas.onKeyDown.listen((evt) {
      if (active) {
        _keyDown(new KeyDownEvent(this, evt.keyCode));
      }
    });
    canvas.onKeyUp.listen((evt) {
      if (active) {
        _keyUp(new KeyUpEvent(this, evt.keyCode));
      }
    });
  }
